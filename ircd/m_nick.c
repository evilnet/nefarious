...
int m_nick(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
    ...
    if (sptr->cli_user->joined > 0) {
      for (lp = sptr->cli_user->channel; lp; lp = lp->next_channel) {
        ...
        if ((member = find_member_link(chptr, sptr))) {
          if (is_ext_banned(sptr, chptr, member, EXTBAN_NICK) && (!is_excepted(sptr, chptr, member, 0) ||
             is_ext_excepted(sptr, chptr, member, EXTBAN_NICK))) {
  ...
  if (IsUnknown(acptr) && MyConnect(acptr)) {
    ++ServerStats->is_ref;
    if (feature_bool(FEAT_IPCHECK) && !find_eline(acptr, EFLAG_IPCHECK))

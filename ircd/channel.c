          ...
          *pe = *ipe_start = 0;
          if (match(tmpe->value.except.exceptstr, ip_se) == 0) {
            if ((bitse > 0) && (bitse < 33)) {
              ...
              *cidre_start = 0;
              except_addr = inet_addr(ipe_start + 1);
              *cidre_start = '/';
...
struct Channel *get_channel(struct Client *cptr, char *chname, ChannelGetType flag)
{
  ...
  if (flag == CGT_CREATE)
  {
    ...
    hAddChannel(chptr);
    if (feature_bool(FEAT_SET_ACTIVE_ON_CREATE) && !IsService(cptr))
...
mode_parse_except(struct ParseState *state, int *flag_p)
{
  ...
  if (*t_str == '~' && t_str[1] && ((t_str[2] == ':') || ((t_str[1] == '!') && (t_str[3] == ':')))
    ...
    switch (t_str[typepos]) {
      case 'q':
        if (!feature_bool(FEAT_EXTEXCEPT_q))
          return;
        if (flags)
      ...
      case 'n':
        if (!feature_bool(FEAT_EXTEXCEPT_n))
          return;
        if (flags)
      ...
      case 'j':
        if (!feature_bool(FEAT_EXTEXCEPT_j))
          return;
        if (flags)
      ...
      case 't':
        if (!feature_bool(FEAT_EXTEXCEPT_t))
          return;
        if (flags)
      ...
      case 'a':
        if (!feature_bool(FEAT_EXTEXCEPT_a))
          return;
        if (flags)
      ...
      case 'c':
        if (!feature_bool(FEAT_EXTEXCEPT_c))
          return;
        if (flags)
      ...
      case 'r':
        if (!feature_bool(FEAT_EXTEXCEPT_r))
          return;
        if (flags)

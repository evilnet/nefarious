...
/*    nextping = CurrentTime; */
  ...
  for (dnsblexempts = DNSBLExemptList; dnsblexempts; dnsblexempts = dnsblexempts->next)
    sendcmdto_one(&me, CMD_EXEMPT, cptr, "%C +%s nb", cptr, dnsblexempts->host);
	...
	sendcmdto_one(cli_user(acptr)->server, CMD_SWHOIS, cptr,
      ...
      if (IsDNSBLMarked(acptr)) /* Burst even if dnsbl is disabled */
        sendcmdto_one(cli_user(acptr)->server, CMD_MARK, cptr, "%s %s %s ma %s :%s", cli_name(acptr),
                      MARK_DNSBL, cli_dnsbl(acptr), cli_user(acptr)->dnsblhost[0]?cli_user(acptr)->dnsblhost[0]:"notmarked", cli_dnsblformat(acptr));

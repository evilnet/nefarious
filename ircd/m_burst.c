...
int ms_burst(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
    ...
    for (param = 3; param < parc; param++) {
	   ...
	   * (unless user is an oper).
          ...
          if (!(check_modes & MODE_KEY)
            ...
            continue;
          sendcmdto_serv_butone(feature_bool(FEAT_HIS_HIDEWHO) ? &his : &me, CMD_KICK, NULL, "%H %C :Net Rider", chptr, member->user);
  ...
  if (!chptr->creationtime || chptr->creationtime > timestamp) {
    ...
    if (*chptr->topic) {
      ...
      *chptr->topic_nick = '\0';
      chptr->topic_time = 0;
      sendcmdto_channel_butserv_butone(feature_bool(FEAT_HIS_HIDEWHO) ? &his : &me, CMD_TOPIC, chptr, NULL, 0, 

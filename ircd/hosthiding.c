/*
 * IRC - Internet Relay Chat, ircd/hosthiding.c
...
/* Regular user host */
void
make_virthost (char *curr, char *host, char *new, char *virt)
{
  ...
  if (parc2 == 4)
      ...
      if (strchr ("0123456789", parv2[3][len - 1]))
	{
	  ...
	  ircd_snprintf (0, mask, HOSTLEN, "%x.%x.%x.IP",
			 hash[0], hash[1], hash[2]);
	}
      else
	{
	  ircd_snprintf (0, mask, HOSTLEN, "%x.%x.%s.%s.%s",
	...
	{
	  ircd_snprintf (0, mask, HOSTLEN, "%x.%x.%s.%s",
			 hash[0], hash[1], parv2[parc2 - 2],
			 parv2[parc2 - 1]);
	}
    }
  strncpy (virt, mask, HOSTLEN);

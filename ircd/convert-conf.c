...
static const char *admin_names[] = { "location", "contact", "contact", 0 },
    ...
    *command_names[] = { "cmd", "service", "prefix", 0 },
    *crule_names[] = { "server", "",  "rule", 0 },
    *dnsbl_names[] = { "server", "name", "flags", "replies", "reply", "rank", 0 },
    *except_names[] = { "mask", "flags", 0 },
...
static void finish_operators(void)
{
    ...
    for (oper = operators; oper; oper = oper->next) {
        fprintf(stdout, "# %s\nOperator {\n\tname = \"%s\";\n"
                ...
                oper->password,
               (strlen(oper->flags) > 0) ? oper->flags : "O",
                oper->class);
        if (oper->is_local) {
            fputs("\tlocal = yes;\n", stdout);
            mask = 1;
...
static void do_port(void)
{
    ...
    case 'X': case 'x': fprintf(stdout, "\texempt = yes;\n"); break;
    case 'E': case 'e': fprintf(stdout, "\tcrypt = yes;\n"); break;  
    }
...
static void do_spoofhost(void)
{
    const char *spoofhost = fields[0], *pass = fields[1], *matchhost = fields[2], *matchident = fields[3];

    if ((pass && pass[0] != '\0') || (matchhost && matchhost[0] != '\0')) {
        /* Print the current line and start the new block. */
        fprintf(stdout, "# %s\nSpoofhost \"%s\" {\n", orig_line, spoofhost);
        ...
        if (matchhost && matchhost[0] != '\0')
            fprintf(stdout, "\thost = \"%s\";\n", matchhost);
        if (matchident && matchident[0] != '\0')
...
static struct string_list *forwards;

static void do_forwards(void)
{
    struct string_list *q;
    q = string_get(&forwards, fields[0]);
    dupstring(q->origin, orig_line);
    dupstring(q->extra, fields[1]);
}

static void finish_forwards(void)
{
    struct string_list *sl;
...
static void do_uworld(void)
{
    ...
    if (fields[1] && fields[1][0])
        fprintf(stdout, "NickJupe { nick = \"%s\"; };\n", fields[1]);
}
...
int main(int argc, char *argv[])
{
    ...
    for (lineno = 1; fgets(line, sizeof(line), ifile); ++lineno) {
        ...
        case 'B':           simple_line("Command", command_names, NULL); break;
        case 'b':           do_forwards(); break;
        ...
        case 'E':           simple_line("Except", except_names, NULL); break;
        ...
        case 'R':           simple_line("Redirect", redirect_names, NULL); break;

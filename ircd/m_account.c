	       ...
	       char* parv[])
{
  ...
  {
    if (strlen(parv[2]) != 1)
      return protocol_violation(cptr, "ACCOUNT detected invalid subcommand token '%s'. Old syntax maybe? See EXTENDED_ACCOUNTS F:line", parv[2] ? parv[2] : "");
	  ...
	  break;
        ...
        case 'H':
          ...
          if (!IsMe(acptr)) {
            /* in-transit message, forward it */
            sendcmdto_one(sptr, CMD_ACCOUNT, acptr,
                          type == 'H' ? "%s %s %s %s %s :%s" : "%s %s %s",
                          parv[1], parv[2], parv[3], parv[4], parv[5], parv[parc-1]);
            return 0;
          } else /* auth checks are for services, not servers */
            return protocol_violation(cptr, "ACCOUNT check (%s %s %s)",
                                      parv[3], parv[4], parv[5]);
       ...
       case 'S':
          ...
          if (!IsMe(acptr)) {
            ...
            sendcmdto_one(sptr, CMD_ACCOUNT, acptr,
                          ...
                          parv[1], parv[2], parv[3], parv[4], parv[5], parv[6], parv[parc-1]);
            return 0;
          } else /* auth checks are for services, not servers */
            return protocol_violation(cptr, "ACCOUNT check (%s %s %s)",
                                      parv[3], parv[4], parv[5]);

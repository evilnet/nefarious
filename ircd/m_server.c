...
check_start_timestamp(struct Client *cptr, time_t timestamp, time_t start_timestamp, time_t recv_time)
{
  ...
  if (feature_bool(FEAT_RELIABLE_CLOCK)) {
    ...
    sendto_opmask_butone(0, SNO_OLDSNO, "got earlier start time: %Tu < %Tu",
...
int mr_server(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
  ...
  if (!(aconf = find_conf_byname(cli_confs(cptr), host, CONF_SERVER))) {
    ++ServerStats->is_ref;
    sendto_opmask_butone(0, SNO_OLDSNO, "Access denied. No conf line for server %s",
                  cli_name(cptr));
    return exit_client_msg(cptr, cptr, &me,
  ...
  if (!verify_sslclifp(cptr, aconf)) {
    sendto_opmask_butone(0, SNO_OLDSNO, "Access denied (SSL fingerprint mismatch) %s",
                  cli_name(cptr));
    return exit_client_msg(cptr, cptr, &me, "No Access (SSL fingerprint mismatch) %s",
                  cli_name(cptr));
  }
